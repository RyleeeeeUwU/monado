...
enum rift_config_report_flags {
	...
	// output the IN report data in the coordinate system used by LibOVR relative to the tracker, otherwise, report in the coordinate system of the device
	RIFT_CONFIG_REPORT_USE_SENSOR_COORDINATES = 1 << 6,
	// override the power state of the USB hub, forcing it to act as if the external power source is connected (DK2 only, does nothing on DK1)
